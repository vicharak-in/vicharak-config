...
Are you sure you want to update the bootloader?"
    ...
    local pid
    pid="$(get_product_id)"
    for i in /lib/u-boot/"$pid"*
    do
        radiolist_add "${i##/lib/u-boot/}" "OFF"
    done
    radiolist_emptymsg "No compatible bootloader is available."

    if ! radiolist_show "Please select the bootloader to be installed:" || (( ${#VICHARAK_CONFIG_RADIOLIST_STATE_NEW[@]} == 0 ))
    then
        return
    fi

    if update_bootloader "$(radiolist_getitem "${VICHARAK_CONFIG_RADIOLIST_STATE_NEW[0]}")"
    then
...
Are you sure you want to update the bootloader?"
    ...
    if update_spinor "$(radiolist_getitem "${VICHARAK_CONFIG_RADIOLIST_STATE_NEW[0]}")"
    then
        msgbox "The bootloader has been updated successfully."
    else
        ret=$?
...
Are you sure you want to update the bootloader?"
    ...
    if update_emmc_boot "$(radiolist_getitem "${VICHARAK_CONFIG_RADIOLIST_STATE_NEW[0]}")"
    then
        msgbox "The bootloader has been updated successfully."
    else
        ret=$?
