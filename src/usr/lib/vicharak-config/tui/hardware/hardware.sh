...
Select any to update their trigger." || (( ${#VICHARAK_CONFIG_CHECKLIST_STATE_NEW[@]} == 0 ))
    then
        return
    fi
    ...
    if ! radiolist_show "Please select the new trigger:" || (( ${#VICHARAK_CONFIG_RADIOLIST_STATE_NEW[@]} == 0 ))
    then
        return
    fi
...
Recommendation: fanless or DC fan => power_allocator | PWM fan => step_wise" || (( ${#VICHARAK_CONFIG_RADIOLIST_STATE_NEW[@]} == 0 ))
    then
        return
    fi

    enable_unique_config set_thermal_governor "$(radiolist_getitem "${VICHARAK_CONFIG_RADIOLIST_STATE_NEW[0]}")"
...
Please check if the screen is connected and powered on."
    ...
    then
        ...
        if ! radiolist_show "Please select the DSI monitor to be mirrored:" || (( ${#VICHARAK_CONFIG_RADIOLIST_STATE_NEW[@]} == 0 ))
        then
            return
        fi
    ...
    then
        ...
        if ! checklist_show "Please select external monitors to mirror the DSI monitor:" || (( ${#VICHARAK_CONFIG_CHECKLIST_STATE_NEW[@]} == 0 ))
        then
            return
        fi
        selected_external=()
