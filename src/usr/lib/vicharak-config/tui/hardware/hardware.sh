...
__hardware_gstreamer_test_picture() {
	...
	if gst-launch-1.0 v4l2src "device=/dev/$VICHARAK_CONFIG_MENU_SELECTED" io-mode=4 ! \
		autovideoconvert ! \
		video/x-raw,format=UYVY,width=1920,height=1080 ! \
		jpegenc ! \
		multifilesink "location=$temp"; then
		msgbox "Test image is saved at $temp."
	else
		rm -f "$temp"
		msgbox "Unable to capture an image with $VICHARAK_CONFIG_MENU_SELECTED device.
Please check if you have the required libraries installed."
	fi
}

__hardware_gstreamer_test_live() {
	gst-launch-1.0 v4l2src "device=/dev/$VICHARAK_CONFIG_MENU_SELECTED" io-mode=4 ! \
		autovideoconvert ! \
...
Select any to update their trigger." || ((${#VICHARAK_CONFIG_CHECKLIST_STATE_NEW[@]} == 0)); then
		return
	fi
	...
	if ! radiolist_show "Please select the new trigger:" || ((${#VICHARAK_CONFIG_RADIOLIST_STATE_NEW[@]} == 0)); then
		return
	fi
	...
	for i in "${VICHARAK_CONFIG_CHECKLIST_STATE_NEW[@]}"; do
		...
		enable_config set_led_trigger "${i[0]}" "$(radiolist_getitem "${VICHARAK_CONFIG_RADIOLIST_STATE_NEW[0]}")"
	done
...
__hardware_thermal() {
	...
	for i in "${available_policies[@]}"; do
		if [[ "$i" == "$current_policy" ]]; then
			radiolist_add "$i" "ON"
		else
			radiolist_add "$i" "OFF"
		fi
...
Recommendation: fanless or DC fan => power_allocator | PWM fan => step_wise" || ((${#VICHARAK_CONFIG_RADIOLIST_STATE_NEW[@]} == 0)); then
		return
	fi

	selected_governor="$(radiolist_getitem "${VICHARAK_CONFIG_RADIOLIST_STATE_NEW[0]}")"
	if enable_unique_config set_thermal_governor "$selected_governor"; then
		msgbox "Thermal governor has been updated."
	else
		if [[ "$selected_governor" == "power_allocator" ]] && (lsmod | grep pwm_fan); then
...
Please check if the screen is connected and powered on."
	...
	if ((${#dsi_monitors[@]} > 1)); then
		radiolist_init
		for i in "${dsi_monitors[@]}"; do
			radiolist_add "$i" "OFF"
		done
		if ! radiolist_show "Please select the DSI monitor to be mirrored:" || ((${#VICHARAK_CONFIG_RADIOLIST_STATE_NEW[@]} == 0)); then
			return
		fi
		selected_dsi="$(radiolist_getitem "${VICHARAK_CONFIG_RADIOLIST_STATE_NEW[0]}")"
	fi

	if ((${#external_monitors[@]} > 1)); then
		checklist_init
		for i in "${external_monitors[@]}"; do
			checklist_add "$i" "OFF"
		done
		if ! checklist_show "Please select external monitors to mirror the DSI monitor:" || ((${#VICHARAK_CONFIG_CHECKLIST_STATE_NEW[@]} == 0)); then
			return
		fi
		selected_external=()
		for i in "${VICHARAK_CONFIG_CHECKLIST_STATE_NEW[@]}"; do
	...
	if ((dsi_native_res_x < dsi_native_res_y)); then
		xrandr_cmd+=(--rotate left)
		dsi_res="${dsi_native_res_y}x${dsi_native_res_x}"
	fi

	for i in "${selected_external[@]}"; do
		xrandr_cmd+=(--output "$i")
	done
	xrandr_cmd+=(
		--auto
