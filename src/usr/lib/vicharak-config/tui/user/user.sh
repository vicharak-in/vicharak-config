...
__user_enable_auto_login (){
    ...
    if ! checklist_show "Please select the interface(s) you want to enable auto login:" || (( ${#VICHARAK_CONFIG_CHECKLIST_STATE_NEW[@]} == 0))
    then
        return
    fi
...
Are you sure to continue?"
    ...
    for selected_tty_shrinked_index in "${VICHARAK_CONFIG_CHECKLIST_STATE_NEW[@]}"
    do
